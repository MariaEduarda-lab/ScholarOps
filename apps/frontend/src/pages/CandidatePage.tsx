...
export function CandidatePage() {
  ...
  return <>
    ...
    {tab === 'summary' && <div className="candidate-layout"><main className="stack-column">
      ...
      <section className="card profile-card"><div className="card-heading"><div><span className="eyebrow">DADOS DE CONTATO</span><h2>Informações principais</h2></div></div><dl><div><dt><Mail />E-mail</dt><dd>{candidate.email}</dd></div><div><dt><Phone />Telefone</dt><dd>{candidate.phone}</dd></div><div><dt><UserRound />Idade</dt><dd>{candidate.age} anos</dd></div></dl><button className="button button--soft button--full"><MessageCircle />Entrar em contato</button></section>
      <section className="card"><span className="eyebrow">CONTEXTO SOCIOECONÔMICO</span><div className="finance-grid"><div><small>Renda familiar</small><strong>{money(candidate.monthlyIncome)}</strong></div><div><small>Renda per capita</small><strong>{money(candidate.perCapitaIncome)}</strong></div><div><small>Núcleo familiar</small><strong>{candidate.familyMembers} pessoas</strong></div><div><small>Documentação</small><strong>{candidate.progress}% completa</strong></div></div><ProgressBar value={candidate.progress} /></section>
    </aside></div>}
    {tab === 'documents' && <section className="card"><div className="card-heading"><div><span className="eyebrow">DOSSÊ DIGITAL</span><h2>Todos os documentos</h2></div><ProgressBar value={candidate.progress} /></div><div className="document-list document-list--all">{candidate.documents.map((document) => <article key={document.id}><span className="file-icon"><FileText /></span><div><strong>{document.label}</strong><small>{document.category} · {document.relationship} · {document.required}</small>{document.issue && <p>{document.issue}</p>}</div><span className="confidence">{Math.round(document.confidence * 100)}%<small>confiança</small></span><StatusBadge status={document.status} /><button className="icon-button" aria-label={`Visualizar ${document.label}`}><ExternalLink /></button></article>)}</div></section>}
    {tab === 'history' && <section className="card"><div className="card-heading"><div><span className="eyebrow">RASTREABILIDADE</span><h2>Histórico da candidatura</h2></div></div><div className="history-list"><div><i /><strong>Candidatura organizada para análise</strong><span>Hoje, 09:40 · ScholarOps</span></div><div><i /><strong>Extração de documentos concluída</strong><span>Ontem, 18:12 · Processamento automático</span></div><div><i /><strong>Documentos enviados pela família</strong><span>{new Date(`${candidate.submittedAt}T12:00`).toLocaleDateString('pt-BR')} · Portal da candidatura</span></div></div></section>}
  </>
}
